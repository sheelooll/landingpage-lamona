/*=========================================
   HERO-SLIDER.JS – Sliders automáticos del index.
   Inicializa TODOS los contenedores .hero-slider
   de la página (el del hero y el de "Nosotros"):
   cada uno busca sus slides, flechas y puntos
   dentro de su propio contenedor, así funcionan
   de forma independiente.

   Cada imagen lleva su data-img-key y se cambia
   desde la sección Imágenes del panel admin
   (Firestore).
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.hero-slider').forEach(iniciarSlider);

    function iniciarSlider(contenedor) {
        const slides  = [...contenedor.querySelectorAll('.hero-slide')];
        const dotsBox = contenedor.querySelector('.hero-slider__dots');
        const prevBtn = contenedor.querySelector('.hero-slider__arrow--prev');
        const nextBtn = contenedor.querySelector('.hero-slider__arrow--next');
